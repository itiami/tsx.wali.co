...
import profileTbl, { IProfile } from '../models/Profile';
...
router.get("/", async (req: Request, res: Response) => {
    const data: Array<any> = await GenericController.findAllByQuery(profileTbl, {});
...
router.post("/findSimilar", async (req: Request, res: Response) => {
    ...
    const data = await GenericController.findAllByQuery(profileTbl, reqPayload);
    res.status(200).json(data);
})
...
router.post("/findOne", async (req: Request, res: Response) => {
    const reqPayload = req.body.filter;
    const data: IProfile = await GenericController.findOne(profileTbl, reqPayload);
    res.status(200).json(data);
})
...
router.post("/count", async (req: Request, res: Response) => {
    const totoal = await GenericController.countDocs(profileTbl, req.body);
...
router.post("/", async (req: Request, res: Response) => {
    const reqPayload = req.body.filter;
    const newCat: IProfile = new profileTbl(reqPayload);

    /* const newCat: IProfile = new profileTbl({
        mainCat: req.body.mainCat,
        subCat: req.body.subCat,
        childCat_1: req.body.childCat_1,
    }); */

    const doc: any = await GenericController.createIfNotExists(profileTbl, reqPayload, newCat);
...
// createDuplIProfilee
router.post("/createDuplIProfilee", async (req: Request, res: Response) => {

    const newCat: IProfile = new profileTbl(req.body.filter);

    const doc: any = await GenericController.createEvenExists(profileTbl, newCat);
...
router.delete("/del", async (req: Request, res: Response) => {
    const reqPayload = req.body.filter;
    const doc = await GenericController.deleteByID(profileTbl, reqPayload);
    res.status(200).json({
...
router.delete("/delMulti", async (req: Request, res: Response) => {
    ...
    try {
        const doc = await GenericController.deleteMulti(profileTbl, reqPayload);
        res.status(200).json(doc);
    } catch (err) {
        console.log(err);
